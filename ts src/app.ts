...
//let a: App;
//let mm: MM;
let TimeoutHideCards: number = 2000;
let TimerHideCards: boolean = false;
let TimeoutIntro;
let ImgTotal: number;
let ShowIntroduction: boolean;
...
class App {
    ...
    cards = [];
    textPlayed: boolean = false;
    allowedCardsUp: number;
    allowedCardsDown: number;
    startCardsUp: number;
    startCardsDown: number;
    cardsUp: number;
    cardsDown: number;
    guessedCards: number;
    cardWidth: number;
    gutterWidth: number;
    upGutterWidth: number;
    ...
    audioForCard: number;
    gameAVMode: number;
    showImageWriteText: number;
    audioTitles = [];
    answers = [];
    timeoutHideCardsUpDown: number;
    //for conceptualization mode define the type of game
    conceptualGame: number;
    imageBox = [];  //will be filled in with image names -> XX.jpg
    imageBoxGroup = []; //every image belong to a group
    imageBoxDescription = []; //every image has its own description 
    showAnswerUp: boolean = false;
    topmostImage = [];
    descriptions = [];
    groupText = [];
    font: string = "Arial";
    ...
    constructor() {
        ...
        let tpImg = GetElements("TopmostImage");
        if (!tpImg)
            this.topmostImage = tpImg.split(";");
        let xModeConcept = String(GetElements("ConceptualGame")).toUpperCase();
        switch (xModeConcept) {
            case "BACKWARDS":
        ...
        let imgInEachGroup = Math.ceil(ImgTotal / this.groups);

        for (let g = 1; g <= this.groups; g++) {
            for (let i = 1; i <= imgInEachGroup; i++) {
                ...
                //assign description
                if (this.writeDescriptions) {
                    let d = GetElements("DescriptionCard" + (g + "-" + i).toString());
                    if (undefined != d)
...
//        Introduction();
    ...
    PrepareCardsUpConceptualization() {

        let existing = [];

        for (let i = 0; i < this.cardsUp; i++) {
            let g = -1;
            ...
            this.cards.push({
                ...
                isDragging: false,
                isHidden: true, //this.gameAVMode > gameAudioVideoModes.None,
...
//                tDescriptions = this.descriptions[this.correctGroups[0]];
    ...
    FindNew(n) {
        ...
        let newItem;
        let name = "OK_" + GetDocURI(); // document['documentURI'];
        let lsItem = localStorage.getItem(name);
        let index = -1;
        if(null == lsItem)
            lsItem = "";

        let itemsAnswered = (lsItem.match(/;/g)||[]).length; 
...
/*                g = Math.floor(Math.random() * ImgTotal);
    ...
    PrepareCardsDown() {
        ...
        for (var g = 0; g < this.correctGroups.length; g++) {
            for (let nCard = 0; nCard < this.correctCardsDown; nCard++) {
        ...
        for (let gCard = this.cards.length - this.cardsUp; gCard < this.cardsDown; gCard++) {
            ...
            this.cards.push({
                ...
                description: this.imageBoxDescription[position],
                group: this.imageBoxGroup[position],
                image: imageName, // this.imageBox[position] + ".jpg",
                isDragging: false,
                isHidden: this.timeoutHideCardsUpDown > 0,
                isDragable: true,
                text: cardText
    ...
    drawCards() {
        ...
        if (0 !== this.topmostImage.length) {
            if (1 == this.topmostImage.length) {
                ...
                imageObj.onload = () => {
                    this.ctx.drawImage(imageObj, this.ctxWidth / 2 - ((this.cardWidth * this.cardsDown) / 2), 0, this.cardWidth * this.cardsDown, this.cardWidth * 0.9);
                }
                imageObj.src = this.topmostImage[0];
                this.ctx.drawImage(imageObj, this.ctxWidth / 2 - ((this.cardWidth * this.cardsDown) / 2), 0, this.cardWidth * this.cardsDown, this.cardWidth * 0.9);
            }
            else {
                for (let i = 0; i < this.cardsUp; i++) {
                    let imageObj = new Image();
                    imageObj.onload = () => {
                        this.ctx.drawImage(imageObj, this.cards[i].x, 0, this.cardWidth, this.cardWidth * 0.9);
                    }
                    imageObj.src = this.topmostImage[this.cards[i].group - 1];
                    this.ctx.drawImage(imageObj, this.cards[i].x, 0, this.cardWidth, this.cardWidth * 0.9);
                }
            }
        }
        ...
        if (undefined !== this.cards[0].description && "" !== this.cards[0].description) {
            ...
            if (0 <= this.cards[0].description.indexOf("#")) {
                ...
                let s: string = this.cards[0].description;
                s = s.replace("#", "              ");
                let widthTitle = this.ctx.measureText(s).width;
            ...
            {
                let s: string[] = this.cards[0].description.split("^");
                let s0: string = s[0];
                let s1: string = s[1];
                ...
                if (0 <= s0.indexOf("%N") || 0 <= s0.indexOf("%n")) {
                    let na = GetElements("Names");
                    let names: string[] = na.split(";");
                    let m = Math.floor(Math.random() * (names.length - 1));
                    let name: string = names[m];

                    if (0 <= s0.indexOf("%N"))
                        s0 = s[0].replace("%N", name[0].toUpperCase() + name.substring(1));
                    if (0 <= s0.indexOf("%n"))
                        s0 = s[0].replace("%n", name);
                    if (0 <= s1.indexOf("%N"))
                        s1 = s[1].replace("%N", name);
                    if (0 <= s1.indexOf("%n"))
                        s1 = s[1].replace("%n", name);
                    ...
                    for (let ixy = 0; ixy < this.cards.length; ixy = ixy + 1) {
                        //console.log(ixy + ": " + this.cards[ixy].text);
                        //tocni odgovor
                        if (0 <= this.cards[ixy].text.indexOf("%N")) {
                            //console.log("Tocno:" + names[m] + " " + name + " " + s0);
                            this.cards[ixy].text = this.cards[ixy].text.replace("%N", name[0].toUpperCase() + name.substring(1));
                        }
                ...
                if (this.gameAVMode < gameAudioVideoModes.Audio || this.gameAVMode == gameAudioVideoModes.AudioVisual) {
                    let widthTitle = this.ctx.measureText(s0).width;
...
/*                    const synUtterance = new window['SpeechSynthesisUtterance']();
                ...
                if(this.gameAVMode < gameAudioVideoModes.Audio)
                {
                    ...
                    let widthTitle = this.ctx.measureText(this.cards[0].description).width;
...
/*                else if(0 <= r.image.indexOf(".mp3"))
                {
                    ...
                    let v: number = parseInt(r.text) + 1;
                    let t: string = this.dirs + v + "-1.jpg";
        ...
        if (z == "blank") {
            ...
            try {
                ...
                imageObj.onload = () => {
                    ...
                    this.ctx.drawImage(imageObj, x, y, w, h);
                }
                imageObj.src = p;
                //console.debug("DI:" + p);
                this.ctx.drawImage(imageObj, x, y, w, h);
            ...
            if (this.writeDescriptions) {
                ...
                let signitureWidth = this.ctx.measureText(z).width;
                this.ctx.fillStyle = "black";
                this.ctx.fillText(z, x + (w - signitureWidth) / 2, y + h * 1.15);
    ...
    isMovingCardOnCardUp(x, y) {
        for (let i = 0; i < this.cardsUp; i++) {

            //get selected card details
            let cUp = this.cards[i];
...
function UnhideCardsAndPlayAudioTitles() {
    ...
    if (a.audioForCard < a.cardsUp) {
        ...
        if (0 > a.cards[a.audioForCard].audio.indexOf(".mp3")) {
                        ...
                        else*/
            {
                ...
                if(a.startCardsUp < a.cardsUp)
                {
                    ...
                    SpeakText(txt);
                    let c = setTimeout(UnhideCardsAndPlayAudioTitles, a.timeoutHideCardsUpDown * 0.75);
                }
                else
                {    
                    ...
                    let c = setTimeout(UnhideCardsAndPlayAudioTitles, a.timeoutHideCardsUpDown);
                }
                a.audioForCard = a.audioForCard + 1;
            }
        }
        //or play mp3 file
            ...
            let audio = new Audio(a.cards[a.audioForCard].audio);
            myPlay(audio);
            a.audioForCard = a.audioForCard + 1;
...
function Introduction() {
    ...
    let titles = GetElements("IntroTitles").split(";");
    let audioIntros = GetElements("AudioIntros").split(";");

    if (!IntroPosition) {

        //initialize array
        for (let j = 0; j < ImgTotal; j++) {
            let m = a.imageBox[j].split("-");
    ...
    if (IntroPosition < IntroImages.length) {
        ...
        let g = IntroImages[IntroPosition].split("-");
        //draw title
        a.ctx.font = "bold " + w * 0.1 + "px Arial";
        let n: number = g[0];
        let z = titles[n - 1];
        let signitureWidth = a.ctx.measureText(z).width;
        a.ctx.fillStyle = "red";
        a.ctx.fillText(z, w - signitureWidth / 2, h * 0.3);

        let text = "";
        let e = document.getElementById("DescriptionCard" + IntroImages[IntroPosition]);
        if (null != e)
            text = e.textContent;
        ...
        let audio = new Audio(audioIntros[n - 1]);
        audio.play();
...
function UpEvent(x, y) {
    ...
    let rec = localStorage.getItem("stat");
    let d = new Date();
    //let dt = d.getDate() + "." + d.getMonth() + "." + d.getFullYear() + ". " + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds();
    let dt = d.getFullYear() + "/" + (d.getMonth() + 1) + "/" + d.getDate() + " " + ('0' + d.getHours()).slice(-2) + ":" + ('0' + d.getMinutes()).slice(-2) + ":" + ('0' + d.getSeconds()).slice(-2);

    // clear all the dragging flags since card is down at the moment
    a.dragOK = false;
    for (let i = 0; i < a.cards.length; i++) {
        a.cards[i].isDragging = false;
    }
    ...
    let i = a.isMovingCardOnCardUp(x, y);
    ...
    if (0 <= i) 
    {
        ...
        if (a.cards[i].group == a.cards[a.movingCardIndex].group || b) 
        {
            ...
            if (ConceptualizationModes.None < a.conceptualGame) {
                ...
                {
                    for (let i = 0; i < a.cardsUp; i++) {
                        let r = Math.random() * a.cardsUp;
...
function MoveEvent(x, y) {
    ...
    if (a.dragOK) {
        ...
        let mx = parseInt(x); // - a.offsetX;
        let my = parseInt(y); // - a.offsetY;
